...
package com.zzz.library_virtual_hosts.vservice;


import com.zzz.library_virtual_hosts.utils.LogUtils;
...
public class DnsChange {
    ...
    public static int handleHosts(String ipValue,String domainValue){

        DOMAINS_IP_MAPS4 = new ConcurrentHashMap<>();
        DOMAINS_IP_MAPS6 = new ConcurrentHashMap<>();

        if (ipValue.contains(":")){
            DOMAINS_IP_MAPS6.put(domainValue + ".",ipValue);
        }else{
            DOMAINS_IP_MAPS4.put(domainValue + ".",ipValue);
        }
        LogUtils.d(TAG, DOMAINS_IP_MAPS4.toString());
        LogUtils.d(TAG, DOMAINS_IP_MAPS6.toString());
        return DOMAINS_IP_MAPS4.size() + DOMAINS_IP_MAPS6.size();
